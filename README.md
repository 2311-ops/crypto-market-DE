...
Data Flow
1) Ingestion
The ingestion service in ingestion/producer.py polls CoinGecko every 60 seconds and publishes a Kafka message for each batch.
...
2) Stream Processing
The Spark job in processing/spark_streaming_job.py consumes raw_crypto_data, flattens the coin array, and writes results to four places:
...
3) Orchestration
The main DAG in orchestration/dags/crypto_pipeline_dag.py is responsible for:

running one ingestion batch
submitting the Spark job on the Spark master container
triggering the data-quality DAG
The data-quality DAG in orchestration/dags/data_quality_dag.py checks:
...
visualization/dashboards/powerbi_queries.sql
visualization/dashboards/powerbi_setup.md
...
Finish the first-time Metabase setup
...
Create questions from the SQL in powerbi_queries.sql
Save the questions
Add them to a dashboard
Refresh the dashboard to see new rows as the pipeline updates Postgres
Power BI
Power BI uses the same Postgres database.

Read the setup guide in visualization/dashboards/powerbi_setup.md
Use the queries in visualization/dashboards/powerbi_queries.sql
For Metabase, replace the :symbol parameter with {{symbol}}
Operational Utilities
storage/minio/setup.py
creates raw-data and processed-data
processing/backfill_minio_to_postgres.py
...
Kafka receives messages from the ingestion container.
Spark writes parquet files to MinIO.
PostgreSQL tables contain rows.
Metabase cards return results.
Airflow logs pipeline activity in pipeline_logs.
Troubleshooting
...
Ingestion publishes a new CoinGecko snapshot every minute.
Airflow runs the Spark job against the latest Kafka backlog.
Spark writes new rows to PostgreSQL and parquet to MinIO.
