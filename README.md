...
- Phase 5: Airflow DAGs orchestrate ingestion + Spark + data quality.
...
3) Run streaming job: see `processing/spark_streaming_job.py` (spark-submit command below).
...
Metabase
- Connect Metabase to Postgres at `localhost:5432`, db `crypto_db`, user `crypto_user`, password `crypto_pass`.
